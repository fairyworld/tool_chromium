...
public class IncognitoTabbedSnapshotControllerIntegrationTest {
    ...
    public void testSecureFlags_incognitoScreenshotDisabled() {
        ...
        assertFalse("Window should not be secure in regular mode", isWindowSecure(activity));

        regularPage.openNewIncognitoTabOrWindowFast();
        assertTrue(
                "Window should be secure when incognito tab is showing", isWindowSecure(activity));

        ThreadUtils.runOnUiThreadBlocking(
                () -> activity.getTabModelSelector().selectModel(/* incognito= */ false));
        assertFalse(
    ...
    public void testSecureFlags_incognitoOverviewScreenshotDisabled() {
        ...
        IncognitoTabSwitcherStation tabSwitcher = incognitoNtp.openIncognitoTabSwitcher();
        assertTrue(
                "Window should be secure when incognito tab switcher is showing",
                isWindowSecure(activity));

        RegularTabSwitcherStation regularTabSwitcher = tabSwitcher.selectRegularTabsPane();
        assertFalse(
